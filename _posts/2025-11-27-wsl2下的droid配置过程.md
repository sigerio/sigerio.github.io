...
`serena`提供了类似于集成开发环境（IDE）能力的必要语义代码检索和编辑工具。
始终扫描使用 `semgrep` 审查新代码的安全漏洞。
当调试的项目是前端项目时，可以使用`chrome-devtools`中的工具进行验证与调试。
始终使用`context7`来查询第三方工具、第三方库的最新使用文档。
始终使用`deepwiki`来查询开源项目的最新文档。
始终优先使用`desktop-commander`中的工具，增强型终端命令。
始终优先使用`github`来查看github上的源码、文件等等资料。
始终优先使用`gitee`来查看gitee上的源码、文件等等资料。
始终使用`fuck-u-code`来检测代码质量。
...
访问 http://localhost:8000/ 然后在key管理界面导入你的API KEY，即可查询当前剩余流量
...
然后选择Add Mcp xxxxxxxxx，查看各个mcp的状态。如果嫌麻烦，请直接咨询droid为什么mcp的配置没有生效（极大概率是工具的路径没有配置正确）。
